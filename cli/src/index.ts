...
const options = yargs
 ...
 .option("l", { alias: "language", describe: "Target language", type: "string", demandOption: true, choices: ['js', 'ts', 'cs'] })
...
const env = nunjucks.configure(path.resolve(__dirname, "./views-" + options.language), { autoescape: true });
env.addFilter('capitalizeFirstLetter', function(val, cb) {
  if(!val) {
    cb(null, val);
    return;
  }
